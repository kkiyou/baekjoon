...
#############################################################################
# 양 쪽 끝에서 한 단위씩 증가시켜가며 진행함.
# 두 말의 거리가 다음 단위의 2배 보다 작거나 같으면 진행을 중지함.
# 다음 단위부터 한 단위씩 낮춰가며 두 말의 거리가 0이 되는 지점을 찾음.
#############################################################################
for _ in range(int(input())):
    ...
    while end - sta > i * 2:
        ...
        i += 1
    count = (i - 1) * 2
    remaining_dis = end - sta
    while remaining_dis != 0:
        if remaining_dis // i > 0:
            count += remaining_dis // i
            remaining_dis %= i
